module AjaxfulRating # :nodoc:
  ...
  module ClassMethods
    ...
    def ajaxful_rateable(options = {})
      has_many :rates_without_dimension, -> { where "dimension IS NULL" }, :as => :rateable, :class_name => 'Rate', :dependent => :destroy
      has_many :raters_without_dimension, :through => :rates_without_dimension, :source => :rater
      
      options[:dimensions].each do |dimension|
        has_many "#{dimension}_rates", :dependent => :destroy, -> { where("dimension = ?", dimension.to_s) }, :class_name => 'Rate', :as => :rateable
